...
- **可安装应用**：支持将 RicebabyVideo 安装为独立应用
...
解决 iOS Safari「添加到主屏幕」后 PWA 与浏览器之间 localStorage 不共享的问题（[#119](https://github.com/18haventgirl/RicebabyVideo/issues/119)），同时实现多设备配置共享（[#115](https://github.com/18haventgirl/RicebabyVideo/issues/115)）。

**工作原理：**

1. 🗄️ **服务端存储（Upstash Redis）**：用户配置通过 `/api/user/config` API 存储在 Upstash Redis 中，使用 `user:config:{profileId}` 作为 key。Edge Runtime 兼容，Cloudflare Pages / Vercel 均可部署。
2. ⬇️ **自动拉取（Pull）**：应用加载时，`useConfigSync` hook 从服务端拉取配置，与本地 `updatedAt` 时间戳比较——服务端更新时自动合并到本地。
3. ⬆️ **自动推送（Push）**：本地设置变更后，自动延迟 3 秒推送到服务端（防抖），避免频繁写入。
...
RicebabyVideo 支持基于环境变量的账户认证系统，支持角色区分和细粒度权限控制。
...
```bash
# Docker
docker run -d -p 3000:3000 -e ADMIN_PASSWORD=your_password --name ricebabyvideo 18haventgirl/ricebabyvideo:latest
```
...
docker run -d -p 3000:3000 \
  ...
  --name ricebabyvideo 18haventgirl/ricebabyvideo:latest
...
```bash
docker run -d -p 3000:3000 -e SUBSCRIPTION_SOURCES='[{"name":"MySource","url":"..."}]' --name ricebabyvideo 18haventgirl/ricebabyvideo:latest
```
...
# Docker
docker run -e PORT=8080 -p 8080:8080 --name ricebabyvideo 18haventgirl/ricebabyvideo:latest
```
...
- **订阅源 (Subscription)**：
  ...
  - 例如：`https://mysite.com/ricebabyvideo-sources.json`
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/18haventgirl/RicebabyVideo)

1. 点击上方按钮
2. 连接你的 GitHub 账号
3. Vercel 会自动检测 Next.js 项目并部署
4. 几分钟后即可访问你自己的 RicebabyVideo 实例
...
2. **创建项目**：
   ...
   - 如果未连接 GitHub，请点击 **Connect GitHub**；若已连接，直接选择你刚刚 Fork 的 `RicebabyVideo` 项目并点击 **Begin setup**。

3. **配置构建参数**：
   - **Project name**: 默认为 `ricebabyvideo` (建议保持不变，后续链接基于此名称)
...
5. **如果部署失败**：
   ...
   >
   > 如果首次部署仍有问题，可尝试在 **[项目设置页面](https://dash.cloudflare.com/?to=/:account/pages/view/ricebabyvideo/settings/production)** 底部的 **Compatibility flags** 中手动添加 `nodejs_compat`，然后重新部署。
...
```bash
# 拉取最新版本
docker pull 18haventgirl/ricebabyvideo:latest
docker run -d -p 3000:3000 --name ricebabyvideo 18haventgirl/ricebabyvideo:latest
```
...
```bash
git clone https://github.com/18haventgirl/RicebabyVideo.git
cd RicebabyVideo
docker build -t ricebabyvideo .
docker run -d -p 3000:3000 --name ricebabyvideo ricebabyvideo
```
...
```bash
# 停止并删除旧容器
docker stop ricebabyvideo
docker rm ricebabyvideo
...
这是网络/反向代理配置问题，非 RicebabyVideo 代码问题。
...
RicebabyVideo 已内置代理服务器自动处理 CORS 问题和 HLS URL 重写，大部分 HLS 直播流应能正常播放。

### IPTV CCTV 等频道只有声音没有画面

部分 CCTV 和卫视频道使用 HEVC (H.265) 编码，某些浏览器不支持硬件解码 HEVC。RicebabyVideo v4.5.0+ 已自动检测 HEVC 流并优先选择 H.264 级别以提高兼容性。如果问题仍存在，建议使用 Chrome 或 Edge 浏览器。
...
RicebabyVideo v4.5.0+ 已增加多级回退机制，会依次尝试 HLS.js、原生 HLS、代理播放等方式。
...
1. **报告 Bug**：[提交 Issue](https://github.com/18haventgirl/RicebabyVideo/issues)
...
- **作者**：[18haventgirl](https://github.com/18haventgirl)
- **项目主页**：[https://github.com/18haventgirl/RicebabyVideo](https://github.com/18haventgirl/RicebabyVideo)
- **问题反馈**：[GitHub Issues](https://github.com/18haventgirl/RicebabyVideo/issues)

---

<div align="center">
  Made with ❤️ by <a href="https://github.com/18haventgirl">18haventgirl</a>
  <br>
